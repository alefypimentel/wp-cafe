...
class Metaboxes
{
	...
	public function add_wp_editor( $post )
	{
		...
		//set id in instance for post
		$this->_instance->ID = $post->ID;
		...
		<div id="apiki-wp-metabox-<?php echo Utils::esc_html( $this->_id ); ?>" class="postbox">
			...
			<div class="inside">
				<?php call_user_func( $this->_wp_editor_callback, $post, $this->_instance ); ?>
			</div>
		</div>
		<?php
	...
	public function content( $post )
	{
		...
		if ( $this->_content_callback && is_callable( $this->_content_callback ) ) :
			call_user_func( $this->_content_callback, $post, $this->_instance );
		endif;
	...
	public function save( $post_id, $post )
	{
		...
		// Tranform meta_keys and merge instance and meta keys
		$this->_meta_keys = $this->get_tranform_keys();

		// Save meta by meta in the array
		array_walk( $this->_meta_keys, array( &$this, 'save_each_postmeta' ) );
	}

	public function get_tranform_keys()
	{
		$metas = array();

		foreach ( $this->_meta_keys as $value ) :
			$this->set_item_in_meta_key( $metas, $value );
		endforeach;

		return $metas;
	}

	public function set_item_in_meta_key( &$metas, $value )
	{
		if ( ! isset( $this->_instance->metas[$value] ) )
			return;

		$args                 = wp_parse_args( $this->_instance->metas[$value], array( 'single' => true ) );
		$new_meta_key         = $this->_post_type . '-' . $value;
		$metas[$new_meta_key] = $args['single'];
	}
